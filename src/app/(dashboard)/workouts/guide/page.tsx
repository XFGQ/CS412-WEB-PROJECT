...
export default function GuidePage() {
  ...
  const categories = ["Tümü", ...Array.from(new Set(exercises.map(e => e.category)))];
  ...
  return (
    <div className="space-y-8 fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">Hareket Rehberi 📚</h1>
          <p className="text-slate-400 mt-1">Formunu düzelt, sakatlanmadan geliş.</p>
        </div>
        ...
        {filteredExercises.map((ex) => (
          <div key={ex.id} className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl hover:border-emerald-500/30 transition-all group flex flex-col h-full">
            ...
            <div className="mt-auto pt-4 border-t border-slate-800/50">
              <p className="text-xs text-slate-500 font-semibold mb-2">PÜF NOKTALARI:</p>

...
export default async function StatsPage() {
  ...
  // Simple Max Value Calculation (For chart scale)
    ...
    <div className="space-y-8 fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl">
          <h3 className="font-bold text-white mb-6">Recent Calorie Intake</h3>
          <div className="h-64 flex items-end justify-between gap-2 px-2">
            {nutrition.length === 0 ? <p className="text-slate-500 w-full text-center self-center">No data</p> : 
              nutrition.map((log, i) => (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-400">
              ...
              <tbody className="divide-y divide-slate-800">
                {workouts.length === 0 ? <tr><td colSpan={4} className="p-4 text-center">No records</td></tr> : 
                  workouts.map((w) => (
                  <tr key={w.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-4 py-3">{w.date || "Today"}</td>
                    <td className="px-4 py-3 font-medium text-white">{w.notes}</td>
                    <td className="px-4 py-3">
                      {w.durationMinutes ? `${w.durationMinutes} min` : `${w.sets} x ${w.reps}`}
                    </td>
